...
### 3. Outbound Traffic Redirection (iptables)
```bash
sudo iptables -t nat -A OUTPUT -p tcp -m owner ! --uuid-owner root --dport 80 -j REDIRECT --to-port 10000
```
* **Purpose**: Redirects *locally originating* outbound traffic to the Envoy proxy.
* **`-t nat -A OUTPUT`**: Unlike `PREROUTING` (which handles incoming traffic), the `OUTPUT` chain handles packets generated locally by processes running on the host.
* **`-p tcp --dport 80`**: Targets outgoing traffic destined for port 80.
* **`-m owner ! --uuid-owner root`**: This is critical. It **excludes** traffic generated by the `root` user from being redirected. 
  * **Why?**: This prevents an infinite loop. If Envoy (running as root) tries to connect to the backend on port 80, it would be redirected back to itself, causing a connection loop.
* **`-j REDIRECT --to-port 10000`**: Redirects the matched outbound traffic to port 10000, where Envoy is listening.

### 4. Why use both PREROUTING and OUTPUT?

| Traffic Type | Chain Used | Goal |
| :--- | :--- | :--- |
| **Incoming from Network** | `PREROUTING` | Force external clients through Envoy. |
